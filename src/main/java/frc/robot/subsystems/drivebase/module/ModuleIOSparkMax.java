...
public class ModuleIOSparkMax implements ModuleIO {
    ...
    public ModuleIOSparkMax(int driveMotorID, int turnMotorID, Rotation2d zeroRotation) {
        ...
        turnConfig
                ...
                .feedForward
                .kV(ModuleConstants.TURN_FF);

        driveConfig
                .idleMode(ModuleConstants.DRIVE_IDLE_MODE)
                .smartCurrentLimit(ModuleConstants.DRIVE_MOTOR_CURRENT_LIMIT);

        turnConfig.idleMode(ModuleConstants.TURN_IDLE_MODE).smartCurrentLimit(ModuleConstants.TURN_MOTOR_CURRENT_LIMIT);
    ...
    public void setTurnPosition(Rotation2d angle) {
        turnPID.setSetpoint(
                SwerveUtils.wrapAngle(angle.minus(zeroRotation).getRadians()), SparkMax.ControlType.kPosition);
    }
