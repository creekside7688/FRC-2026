...
public class TestSOTM extends Command {
    ...
    public TestSOTM(
            Shooter shooter,
            ShooterFeeder feeder,
            ShooterHood shooterHood,
            Spindexer spindexer,
            SwerveDrive sd,
            Supplier<Double> getControllerX,
            Supplier<Double> getControllerY) {
    ...
    public void execute() {
        ...
        double angle = SwerveUtils.lookAtPoint(
        ...
        Translation2d latencyOffset = ShooterLookup.CalculationDelayOffset(
                sd.getPose(), sd.getChassisSpeeds().vxMetersPerSecond, sd.getChassisSpeeds().vyMetersPerSecond, 0.02);

        distance += latencyOffset.getX();
        angle += latencyOffset.getY();

        Translation2d predictedTarget = ShooterLookup.RecurssiveFOT(
                sd.getChassisSpeeds().vxMetersPerSecond,
                sd.getChassisSpeeds().vyMetersPerSecond,
                shooterHood.getHoodPosition(),
                angle,
                distance);
        ...
        CommandScheduler.getInstance()
                .schedule(DriveCommands.joystickDriveWithRotationalOverride(
                        sd, getControllerX::get, getControllerY::get, () -> Rotation2d.fromDegrees(finalAngle)));

        if (shooter.checkShooterRPMTolerance()
                && shooterHood.checkShooterPositionTolerance()
                && sd.getPose().getRotation().getDegrees() <= finalAngle + 1
                && sd.getPose().getRotation().getDegrees() >= finalAngle - 1) {
            feeder.RunFeeder();
